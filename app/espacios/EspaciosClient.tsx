...
const services = [
  ...
  },
  {
    ...
    hours: null,
    es: {
      name: "Tours y Experiencias Locales",
    ...
    en: {
      name: "Local Tours and Experiences",
...
export default function EspaciosClient() {
  const { lang } = useLang();
    ...
    <>
      ...
      <section className="relative h-64 md:h-80 flex items-end pb-10 px-4 overflow-hidden">
        <div className="absolute inset-0">
          <Image
            src="/svc-piscina.jpg"
            alt="Piscina jacuzzi y servicios La Guaca Hostel Santa Marta"
            fill
            priority
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full">
          ...
          <h1 className="font-syne font-bold text-4xl md:text-5xl text-white">
            {lang === "es" ? "Servicios y Amenidades · La Guaca Hostel" : "Hostel Amenities Santa Marta"}
          </h1>
        </div>
      </section>
          ...
          {services.map((svc) => (
            ...
            >
              ...
              <span className="font-inter text-[10px] text-grafito-muted group-hover:text-terracota transition-colors text-center leading-tight max-w-[72px]">
                {svc[lang].name.split("—")[0].trim().split(" ").slice(0, 2).join(" ")}
              </span>
            </button>
          ))}
        </div>
        ...
        {services.map((svc, idx) => (
          ...
          >
            ...
            <div className="w-full lg:w-1/2 relative h-72 md:h-96 rounded-card overflow-hidden">
              <Image
                src={svc.img}
                alt={`${svc[lang].name} en La Guaca Hostel Santa Marta`}
                fill
                className="object-cover"
              />
              {svc.hours && (
